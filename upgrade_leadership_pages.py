...
def get_lawyer_sidebar(district_name):
    # Lawyer Card customized for parents trying to escalate issues with district leadership
    return f"""
    <div class="premium-sidebar-right" style="flex: 1 1 30%; min-width: 300px; position: sticky; top: 20px;">
        <div style="background: #fdfaf5; border: 1px solid #d6cbbf; border-radius: 6px; overflow: hidden; font-family: 'DM Sans', sans-serif; box-shadow: 0 10px 25px rgba(0,0,0,0.08);">
           ...
           <div style="padding: 24px; background: #faf7f2;" id="leadershipLeadFormContainer">
              <p style="font-family: 'Cormorant Garamond', serif; font-size: 19px; font-weight: 600; color: #1a1410; line-height: 1.3; margin-top: 0; margin-bottom: 8px;">
                 Not getting answers from the district?
              </p>
              <p style="font-size: 13px; line-height: 1.5; color: #6b5f53; margin-bottom: 20px;">
                 If district leadership is ignoring your requests, it may be time to escalate. Request a free 15-minute legal consultation to discuss your next steps.
              </p>

              <form id="leadershipLeadForm" style="display: flex; flex-direction: column; gap: 14px;">
                 ...
                 <div>
                     ...
                     <select id="leadIssue" required style="width: 100%; padding: 12px; border: 1px solid #d6cbbf; font-size: 13px; color: #1a1410; background-color: #fff; cursor: pointer; box-sizing: border-box; font-family: 'DM Sans', sans-serif; border-radius: 4px; -webkit-appearance: none; background-image: url('data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2210%22%20height%3D%226%22%3E%3Cpath%20d%3D%22M0%200l5%206%205-6z%22%20fill%3D%22%23b8963a%22%2F%3E%3C%2Fsvg%3E'); background-repeat: no-repeat; background-position: right 12px center;">
                         ...
                         <option>District is ignoring my emails/calls</option>
                         <option>CSE meeting was denied or delayed</option>
                         <option>I need to file a formal State Complaint</option>
                         <option>Due Process / Impartial Hearing</option>
                         <option>Other</option>
                     </select>
                 </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            const form = document.getElementById('leadershipLeadForm');
            const submitBtn = document.getElementById('leadSubmitBtn');
            const container = document.getElementById('leadershipLeadFormContainer');
...
def main():
    print("Scanning for leadership-directory.html files...")
    ...
    for root, dirs, files in os.walk('.'):
        ...
        if 'districts' in parts and parts[-1] != 'districts':
            if 'leadership-directory.html' in files:
                files_to_process.append(os.path.join(root, 'leadership-directory.html'))
                
    if not files_to_process:
        print("ERROR: Could not find any district leadership-directory.html files.")
        return
    ...
    for file_path in files_to_process:
        ...
        if title_tag:
            ...
            if 'Contacts:' in title_text:
                district_name = title_text.split('Contacts:')[-1].strip()
            elif '|' in title_text:
    ...
    print(f"\n======================================")
    print(f"DONE! Upgraded {count} Leadership pages.")
    print(f"======================================")
