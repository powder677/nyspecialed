...
def get_lawyer_sidebar(district_name):
    ...
    <div class="premium-sidebar-right" style="flex: 1 1 30%; min-width: 300px; position: sticky; top: 20px;">
        <div style="background: #fdfaf5; border: 1px solid #d6cbbf; border-radius: 6px; overflow: hidden; font-family: 'DM Sans', sans-serif; box-shadow: 0 10px 25px rgba(0,0,0,0.08);">
           ...
           <div style="padding: 24px; background: #faf7f2;" id="advocacyLeadFormContainer">
              <p style="font-family: 'Cormorant Garamond', serif; font-size: 19px; font-weight: 600; color: #1a1410; line-height: 1.3; margin-top: 0; margin-bottom: 8px;">
                 Advocating for your child can be overwhelming.
              </p>
              <p style="font-size: 13px; line-height: 1.5; color: #6b5f53; margin-bottom: 20px;">
                 You have equal rights at the CSE table. Request a free 15-minute legal consultation to discuss your child's IEP and ensure they get the services they deserve.
              </p>

              <form id="advocacyLeadForm" style="display: flex; flex-direction: column; gap: 14px;">
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            const form = document.getElementById('advocacyLeadForm');
            const submitBtn = document.getElementById('leadSubmitBtn');
            const container = document.getElementById('advocacyLeadFormContainer');
...
def main():
    print("Scanning for parent-advocacy-guide.html files...")

    files_to_process = []
    for root, dirs, files in os.walk('.'):
        parts = root.split(os.sep)
        if 'districts' in parts and parts[-1] != 'districts':
            if 'parent-advocacy-guide.html' in files:
                files_to_process.append(os.path.join(root, 'parent-advocacy-guide.html'))
                
    if not files_to_process:
        print("ERROR: Could not find any district parent-advocacy-guide.html files.")
        return
    ...
    for file_path in files_to_process:
        ...
        # Locate the content container (either <div class="content-body"> or <main>)
        content_div = soup.find('div', class_='content-body')
        main_tag = soup.find('main')
        
        target_container = content_div if content_div else main_tag
        ...
        if title_tag:
            ...
            # Title format is typically: "CSE Advocacy Guide for Central Islip | New York Special Ed"
            if ' for ' in title_text and '|' in title_text:
                district_name = title_text.split(' for ')[1].split('|')[0].strip()
                # Clean up if it grabbed "District Name Parents" or similar
                if district_name.endswith('Parents'):
                    district_name = district_name.replace(' Parents', '').strip()
            elif '|' in title_text:
                district_name = title_text.split('|')[0].strip()
                    
        # Check if it's already inside a flex container from a previous pass
        split_container = target_container.find('div', class_='premium-split-container')
        if not split_container and target_container.parent and 'premium-split-container' in target_container.parent.get('class', []):
            split_container = target_container.parent
        ...
        if split_container:
            ...
            # If target is main, ensure it isn't restricted to 900px wide
            if target_container.name == 'main':
                if 'container' in target_container.get('class', []):
                    target_container['class'].remove('container')
                target_container['style'] = 'padding: 40px 20px; width: 100%; box-sizing: border-box;'
            ...
            old_class = ' '.join(target_container.get('class', [])) if target_container.name != 'main' else 'premium-content-left'
            left_col = soup.new_tag('div', **{'class': old_class, 'style': 'flex: 1 1 60%; min-width: 300px; max-width: 100%; padding: 0; margin-top: 0;'})
            
            # Move all existing contents into the left column
            for child in list(target_container.contents):
            ...
            target_container.append(split_section)
    ...
    print(f"\n======================================")
    print(f"DONE! Upgraded {count} Advocacy Guide pages.")
    print(f"======================================")
